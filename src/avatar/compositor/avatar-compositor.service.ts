...
private buildCompositionArgs(options: CompositorOptions): string[] {
  ...
  const args = [
    '-y',
    '-i', backgroundVideoPath,    // [0:v] Background
    '-i', avatarForegroundPath,   // [1:v] Avatar (with alpha)
    '-i', audioPath,               // [2:a] Main audio
  ];
  ...
  // ============================================
  // 🔥 FIX: Proper alpha blending overlay
  // ============================================
  // Key change: format=auto handles alpha properly
  // ycbcr forces RGB mixing, not alpha compositing
  let filterComplex = '[0:v][1:v]overlay=0:0:format=auto:alpha=premultiplied[outv]';
  ...
  args.push(
    ...
    '-preset', 'medium',
    '-crf', '23',
    '-pix_fmt', 'yuv420p',  // Final output doesn't need alpha
    '-c:a', 'aac',
    '-b:a', '192k',
    '-movflags', '+faststart',
    '-shortest',
  ...
  console.log(`   ✅ Composition command ready (with alpha blending)`);
  return args;
}
...
//   const args = [
//     '-y',
//     '-i', backgroundVideoPath,  // [0:v] - Background VIDEO ONLY (no audio)
//     '-i', audioPath,             // [1:a] - Main audio
//   ];

//   if (backgroundMusicPath && fs.existsSync(backgroundMusicPath)) {
//     args.push('-i', backgroundMusicPath); // [2:a] - Background music
//     args.push(
//       '-filter_complex', 
//       // ✅ FIX: Use [1:a] and [2:a] instead of [0:a] (which doesn't exist)
...
//     );
//   } else {
//     // Only main audio, no background music
//     args.push(
//       '-map', '0:v:0',   // Map video from input 0
...
private buildAudioOnlyArgs(
  ...
  const args = [
    '-y',
    '-i', backgroundVideoPath,  // [0:v] - Video only (NO audio)
    '-i', audioPath,             // [1:a] - Main audio
  ];
  ...
  if (backgroundMusicPath && fs.existsSync(backgroundMusicPath)) {
    ...
    args.push(
      '-map', '0:v:0',   // Video from background
      '-map', '1:a:0'    // Audio from main audio file
    );
  }
...
//     // Ensure avatar stream is in RGBA so alpha is preserved, then overlay
//     let filterComplex = '[1:v]format=rgba[fg];[0:v][fg]overlay=0:0:eval=init:shortest=1[outv]';
//     let audioFilter = '';
